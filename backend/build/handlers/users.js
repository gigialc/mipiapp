...
function mountUserEndpoints(router) {
    ...
    router.post('/update', async (req, res) => {
        ...
        const updatedUser = await userCollection.findOneAndUpdate({ uid: currentUser.uid }, { $set: { username: username, bio: bio, coinBalance: coinbalance } }, { new: true, returnDocument: 'after' });
        if (!updatedUser) {
    ...
    router.get('/me', async (req, res) => {
        try {
            ...
            const communities = await Promise.all(currentUser.communitiesCreated.map(async (communityId) => {
                // Directly find a single community by _id
                const community = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(communityId) });
    ...
    router.get('/joined', async (req, res) => {
        try {
            ...
            const communityCollection = req.app.locals.communityCollection;
            const communities = await Promise.all(currentUser.communitiesJoined.map(async (communityId) => {
                const community = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(communityId) });
                return community; // May return null if not found
            }));
            const communityMap = communities.filter(c => c).map((community) => ({
    ...
    router.post('/addUser', async (req, res) => {
        try {
            ...
            const userCollection = req.app.locals.userCollection;
            const updatedUser = await userCollection.findOneAndUpdate({ uid: userId }, { $addToSet: { communitiesJoined: communityId } }, // Use communityId directly if it's stored as a string
