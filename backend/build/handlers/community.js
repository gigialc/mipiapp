...
function mountCommunityEndpoints(router) {
    ...
    router.post('/create', async (req, res) => {
        try {
            ...
            const updateResult = await userCollection.updateOne({ _id: creatorData._id }, { $push: { communitiesCreated: newCommunity._id } });
    ...
    router.post('/posts', async (req, res) => {
        ...
        try {
            ...
            const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(communityId) }, { $push: { posts: post } });
            ...
            const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(communityId) });
    ...
    router.get('/community/:id', async (req, res) => {
        ...
        try {
            const community = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
            if (!community) {
    ...
    router.put('/community/:id', async (req, res) => {
        ...
        const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: community });
        const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
        return res.status(200).json({ updatedCommunity });
    });
    router.delete('/community/:id', async (req, res) => {
        const communityCollection = req.app.locals.communityCollection;
        const id = req.params.id;
        const deleteResult = await communityCollection.deleteOne({ _id: new mongodb_1.ObjectId(id) });
    ...
    router.post('/community/:id/leave', async (req, res) => {
        ...
        const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $pull: { members: user.uid } });
        const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
        return res.status(200).json({ updatedCommunity });
    });
    router.post('/community/:id/post', async (req, res) => {
        ...
        const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $push: { posts: post } });
        const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
        return res.status(200).json({ updatedCommunity });
    });
    router.post('/community/:id/admin', async (req, res) => {
        ...
        const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $push: { admins: user.uid } });
        const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
        return res.status(200).json({ updatedCommunity });
    });
    router.post('/community/:id/unadmin', async (req, res) => {
        ...
        const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $pull: { admins: user.uid } });
        const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
        return res.status(200).json({ updatedCommunity });
    });
    router.post('/community/:id/invite', async (req, res) => {
        ...
        const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $push: { invited: user.uid } });
        const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
        return res.status(200).json({ updatedCommunity });
    });
    router.post('/community/:id/uninvite', async (req, res) => {
        ...
        const updateResult = await communityCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $pull: { invited: user.uid } });
        const updatedCommunity = await communityCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
        return res.status(200).json({ updatedCommunity });
    });
