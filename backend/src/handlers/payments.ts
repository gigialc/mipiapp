...
export default function mountPaymentsEndpoints(router: Router) {
  ...
  */ 
  ...
  router.post('/cancelled_payment',  authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    const app = req.app;
  ...
  */ 
  ...
  router.post('/refundable_payment',  authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  ...
  router.post('/refundable_payment/refund_payment', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    if (!req.session.currentUser) { 
      return res.status(401).json({ error: 'unauthorized', message: "User needs to sign in first" });
    }
